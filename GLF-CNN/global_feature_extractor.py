...
class FrequencyDomainFeatureExtractor(tf.keras.Model):
    def __init__(self):
...
def extract_features(data):
    model = FrequencyDomainFeatureExtractor()
    return model(data)
...
def main():
    ...
    for i in range(1, 28):
        ...
        new_directory = os.path.join(base_dir, f's{i}', f's{i}_global_features')
        ...
        for j in range(1, 4):
            ...
            data_tensor = tf.convert_to_tensor(data_fft, dtype=tf.float32)
            data_tensor = tf.transpose(data_tensor, perm=[0, 2, 1])
            features = extract_features(data_tensor)
            
            # Save the features
            output_file_path = os.path.join(new_directory, f's{i}_A1_E{j}_global_features.mat')
            sio.savemat(output_file_path, {'global_features': features.numpy()})
            print(f"Global features saved to {output_file_path}")
