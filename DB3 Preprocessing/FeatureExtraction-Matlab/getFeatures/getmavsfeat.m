%
% GETWLFEAT Gets the Mean Absolute Value Slope feature.
%
...
function feat = getmavsfeat(x,winsize,wininc,datawin,dispstatus)
...
for i = 1:numwin-1,
   ...
   curwinSucc = x(st + wininc:en + wininc,:).*repmat(datawin,1,Nsignals);

%    feat(i,:) = sum(abs(diff(curwin,2)));
   feat(i,:)=(mean(abs(curwinSucc))-mean(abs(curwin)));
